...
print(f"This script will mass delete the Jamf server: {JAMF_HOSTNAME} (using the name {JAMF_USERNAME})!!!")
...
run_command(0, *confpythonjamf, "-u", "wrong", "-p", "wrong", "-c", "no", "-H", JAMF_HOSTNAME)
run_command(0, *confpythonjamf, "-P")
run_command(1, *confpythonjamf, "-t")

print("#################################################################################")
print("Testing could not connect")
run_command(0, *confpythonjamf, "-u", "wrong", "-p", "wrong", "-c", "no", "-H", "http://wrong")
run_command(0, *confpythonjamf, "-P")
run_command(1, *confpythonjamf, "-t")

print("#################################################################################")
print("Testing connection successful")
run_command(0, *confpythonjamf, "-u", JAMF_USERNAME, "-p", JAMF_PASSWORD, "-c", "no", "-H", JAMF_HOSTNAME)
run_command(0, *confpythonjamf, "-P")
...
run_command(0, *jctl, "computergroups", "--use-the-force-luke", "-j", "-c", '{"name": "staticComputerGroup1", "is_smart": "false", "computers": {"computer": [{"name": "computer1"}]}}')
run_command(0, *jctl, "computergroups", "--use-the-force-luke", "-j", "-c", '{"name": "Needs Zoom 5.11.11", "is_smart": "true", "criteria": {"size": "1", "criterion": [{"name": "Computer Name", "priority": "0", "and_or": "and", "search_type": "is", "value": "computer1", "opening_paren": "false", "closing_paren": "false"}]}}')

# #run_command(0, *jctl, "scripts", "--use-the-force-luke", "-j", "-c", '{"name":"test_script","script_contents":"#!/bin/sh\n\necho hello world\n"}')
#
# ##########################################################################################
#
run_command(0, *jctl, "packages", "--use-the-force-luke", "-c", "Zoom-5.11.11 (10514).pkg")
run_command(0, *jctl, "packages", "--use-the-force-luke", "-c", "Zoom-5.11.10 (10279).pkg")
run_command(0, *jctl, "packages", "--use-the-force-luke", "-c", "Zoom-5.11.9 (10046).pkg")

run_command(0, *jctl, "policies", "--use-the-force-luke", "-c", "Install Zoom1")
run_command(0, *jctl, "policies", "--use-the-force-luke", "-r", "Install Zoom1", "-j", "-u", 'package_configuration={"packages": {"package": [{"name": "Zoom-5.11.11 (10514).pkg", "action": "Install"}]}}')

result = run_command(None, *jctl, "patchsoftwaretitles", "--use-the-force-luke", "-j", "-c", '{"name": "Zoom Client for Meetings", "name_id": "0F9", "source_id": "1"}')

if (result == 0):
    zoomid = subprocess.check_output(
        [*jctl, "patchsoftwaretitles", "-r", "Zoom Client for Meetings", "-I"],
        cwd=REPO_ROOT,
    ).decode().strip()
    run_command(0, *pkgctl, "-p")
    run_command(0, *jctl, "patchpolicies", "--use-the-force-luke", "-j", "-c", '{"general": {"name": "Zoom 1","target_version": "5.11.10 (10279)"},"software_title_configuration_id": "' + zoomid + '"}')
    run_command(0, *jctl, "patchpolicies", "--use-the-force-luke", "-j", "-c", '{"general": {"name": "Zoom 2","target_version": "5.11.11 (10514)"},"software_title_configuration_id": "' + zoomid + '"}')
else:
